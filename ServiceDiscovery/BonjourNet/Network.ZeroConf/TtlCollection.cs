...
{
    internal class TtlCollection<T> : ICollection<T>
        where T : IExpirable
    {
        ...
        {
            DateTime expiration = DateTime.UtcNow.AddSeconds(item.Ttl);
        ...
        private List<T> EnsureUpToDate()
        {
            ...
            while (expirables.First().Key < DateTime.UtcNow)
            {
        ...
        public DateTime NextExpiration
        {
            ...
            set
            {
                ...
                timerBeforeNextExpiration.Change((DateTime.UtcNow - nextExpiration), new TimeSpan(0, 0, 0, 0, -1));
            }
        }
