...
using EndPoint = Network.Dns.EndPoint;
...
namespace Network.Bonjour
{
    internal class Service : IService
    {
        ...
        public bool IsOutDated { get { return DateTime.UtcNow > expiration; } }
        ...
        {
            var endpoint = this.Addresses.FirstOrDefault();
            if (endpoint == null) return null;
            if (!endpoint.Addresses.Any()) return null;

            // only return the first IP
            endpoint.Addresses = endpoint.Addresses.Take(1).ToList();
            return endpoint;
        }
            ...
            switch (a.Type)
            {
                ...
                case Network.Dns.Type.SRV:
                    ...
                    if (addresses.Count == 0)
                        AddAddress(new Network.Dns.EndPoint()
                        {
                            DomainName = a.DomainName, //HostName, 
        ...
        public uint Ttl
        {
            get
            {
                int seconds = (int)expiration.Subtract(DateTime.UtcNow).TotalSeconds;
                if (seconds < 0)
                    return 0;
                return (uint)seconds;
            }
